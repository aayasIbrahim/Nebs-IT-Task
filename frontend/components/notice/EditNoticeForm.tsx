...
import { EditSubmitButton } from "./EditSubmitBtn";
...
export default function EditNoticeForm({ notice }: { notice: Notice }) {
  ...
  const [noticeTypes, setNoticeTypes] = useState<string[]>(
    Array.isArray(notice.noticeType)
      ? notice.noticeType
      : [notice.noticeType].filter(Boolean)
  );
  ...
  return (
    <form action={handleSubmit} className="space-y-8 max-w-5xl mx-auto pb-20">
      {/* Header Info Card */}
      <div className="flex flex-col md:flex-row items-center justify-between bg-gradient-to-r from-yellow-600 to-yellow-700 p-6 rounded-[2rem] text-white shadow-lg">
      ...
      <div className="bg-white p-8 md:p-12 rounded-[3rem] border border-slate-100 shadow-2xl space-y-12">
        ...
        <div className="space-y-8">
          <div className="flex items-center gap-3 text-slate-800 font-black uppercase text-xs tracking-widest">
            <span className="flex items-center justify-center w-6 h-6 rounded-full bg-yellow-900 text-white text-[10px]">
              1
            </span>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2 p-3 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                {noticeTypes.map((tag) => (
                  <span
                    key={tag}
                    className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-600 shadow-sm"
                  >
                    {tag}
                    <button
                      type="button"
        ...
        {/* Section 2: Author Details (NEWLY ADDED) */}
        <div className="space-y-8">
          <div className="flex items-center gap-3 text-slate-800 font-black uppercase text-xs tracking-widest">
            <span className="flex items-center justify-center w-6 h-6 rounded-full bg-blue-600 text-white text-[10px]">
              2
            </span>
            Author Information
          </div>
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full h-14 pl-12 pr-6 bg-slate-50 rounded-2xl outline-none font-bold text-slate-600 text-sm border-2 border-transparent focus:border-blue-100"
                  placeholder="EMP-000"
                />
              </div>
            </div>
        ...
        <div className="space-y-8">
          <div className="flex items-center gap-3 text-slate-800 font-black uppercase text-xs tracking-widest">
            <span className="flex items-center justify-center w-6 h-6 rounded-full bg-yellow-400 text-white text-[10px]">
              3
            </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-3">
              ...
              <select
                ...
                className="w-full h-16 px-6 bg-slate-50 rounded-2xl outline-none font-bold text-slate-600 appearance-none shadow-sm cursor-pointer"
              >
            ...
            <div className="space-y-3">
              ...
              <input
                ...
                name="publishDate"
                defaultValue={
                  notice.publishDate ? notice.publishDate.split("T")[0] : ""
                }
                className="w-full h-16 px-6 bg-slate-50 rounded-2xl outline-none font-bold text-slate-600 shadow-sm"
              />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-8">
          <div className="flex items-center gap-3 text-slate-800 font-black uppercase text-xs tracking-widest">
            <span className="flex items-center justify-center w-6 h-6 rounded-full bg-yellow-600 text-white text-[10px]">
              4
            </span>
          ...
          <div className="p-6 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200 group hover:border-blue-400 transition-colors">
            <label className="flex flex-col items-center justify-center cursor-pointer gap-2">
              ...
              <span className="text-[10px] text-slate-400 font-bold italic">
                Current:{" "}
                {notice.attachmentUrl ? "document_attached.pdf" : "No file"}
              </span>
      ...
      <div className="flex flex-col-reverse sm:flex-row items-center gap-4 w-full mt-8">
        <button
          type="button"
          onClick={() => router.back()}
          className="w-full sm:w-auto px-10 h-16 bg-white border border-slate-200 rounded-2xl font-bold text-[12px] uppercase tracking-widest text-slate-400 hover:bg-slate-50 transition-all"
        >
          Discard
        </button>
        <EditSubmitButton />
