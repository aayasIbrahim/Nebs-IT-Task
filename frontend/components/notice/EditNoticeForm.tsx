...
export default function EditNoticeForm({ notice }: { notice: Notice }) {
  ...
  <button
    ...
    disabled={loading}
    onClick={() => router.back()}
    className="w-full sm:w-auto px-10 h-16 bg-white border border-slate-200 rounded-2xl font-bold text-[12px] uppercase tracking-widest text-slate-400 hover:bg-slate-50 hover:text-slate-600 transition-all disabled:opacity-50"
  >
    Discard
  </button>

  {/* Push Updates (Submit) Button */}
  <button
    type="submit"
    disabled={loading}
